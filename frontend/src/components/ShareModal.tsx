    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-[420px] bg-white dark:bg-neutral-900 rounded-2xl border-2 border-black dark:border-neutral-700 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_0px_rgba(255,255,255,0.08)] flex flex-col animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 flex items-center justify-between border-b-2 border-black dark:border-neutral-700">
          <h2 className="text-base font-bold text-slate-800 dark:text-neutral-100 truncate pr-4" title={drawingName}>
            Share "{drawingName}"
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 px-6 py-5 space-y-5 overflow-visible">
              ...
              <div className="absolute top-full left-0 right-0 mt-2 border-2 border-black dark:border-neutral-700 rounded-xl bg-white dark:bg-neutral-900 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_0px_rgba(255,255,255,0.08)] overflow-hidden z-[200] animate-in fade-in slide-in-from-top-2">
                {userResults.map((u) => (
                  <button
                    ...
                    className="w-full text-left px-4 py-2 flex items-center gap-3 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors group border-b last:border-b-0 border-slate-100 dark:border-neutral-800"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-bold text-slate-900 dark:text-neutral-100 truncate">{u.name}</div>
                      <div className="text-[10px] font-semibold text-slate-500 dark:text-neutral-400 truncate">{u.email}</div>
                    </div>
                    <Plus size={16} className="text-slate-400 group-hover:text-indigo-600 transition-colors" strokeWidth={2} />
                  </button>
                ))}
              </div>
            )}
          ...
          <section className="space-y-2">
            <h3 className="text-xs font-bold text-neutral-500 dark:text-neutral-400 px-1">People with access</h3>
            ...
            <div className="space-y-0.5">
              <div className="flex items-center gap-3 px-1 py-1.5">
                ...
                <div className="text-xs font-semibold text-neutral-400 dark:text-neutral-500 pr-1 shrink-0">Owner</div>
              </div>
              ...
              {(sharing?.permissions || []).map((p) => (
                <div key={p.id} className="flex items-center gap-3 px-1 py-1.5 group">
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-center">
                    <div className="text-xs font-bold text-slate-900 dark:text-neutral-100 leading-tight truncate">{p.granteeUser.name}</div>
          ...
          {/* General access */}
          <section className="pt-4 border-t-2 border-black dark:border-neutral-700">
            <h3 className="text-xs font-bold text-neutral-500 dark:text-neutral-400 px-1 mb-2.5">General access</h3>
            ...
            <div className="flex items-start gap-3.5 px-1">
              <div className={clsx(
                ...
                {activeLink ? <Globe size={18} strokeWidth={2} /> : <Lock size={18} strokeWidth={2} />}
              </div>
              ...
              <div className="flex-1 min-w-0 flex flex-col gap-0">
                ...
                <p className="text-[11px] font-medium text-slate-500 dark:text-neutral-400 leading-snug px-0.5">
                  {activeLink ? "Anyone on the internet with the link can access." : "Only people with access can open with the link."}
                </p>

                {activeLink && (
                  <div className="pt-3.5 space-y-3.5 animate-in fade-in slide-in-from-top-1 duration-200">
                    <p className="text-[10px] font-semibold text-slate-500 dark:text-neutral-400 px-0.5">
                      {formatAutoDisableText(activeLink.expiresAt)} When it disables, General access switches back to Restricted.
                    </p>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <CustomSelect
                        ...
                        icon={<Shield size={12} strokeWidth={2} className="text-slate-400" />}
                        variant="bordered"
                      />
                      <CustomSelect
                        value={expiryOption}
                        ...
                        icon={<Calendar size={12} strokeWidth={2} className="text-slate-400" />}
                        variant="bordered"
                      />
                    </div>
                    ...
                    {expiryOption === "custom" && (
                      <input
                        ...
                        className="w-full px-3 py-1.5 rounded-xl border-2 border-black dark:border-neutral-700 bg-slate-50 dark:bg-neutral-800 text-xs font-semibold focus:outline-none focus:border-indigo-600 transition-all shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_0px_rgba(255,255,255,0.05)]"
                      />
                    )}

                    {linkPermission === "edit" && (
                      <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-900/10 border-2 border-amber-500 space-y-1 shadow-[2px_2px_0px_0px_rgba(245,158,11,0.2)]">
                        <div className="flex items-start gap-2">
                          <AlertTriangle size={14} strokeWidth={2} className="text-amber-600 shrink-0 mt-0.5" />
                          <div className="text-[10px] text-amber-900 dark:text-amber-200 font-semibold leading-relaxed">
                            <span className="font-bold text-[11px] text-amber-800 dark:text-amber-300">Security Warning</span><br/>
        ...
        {isLoading && (
          <div className="absolute inset-0 bg-white/20 dark:bg-black/10 backdrop-blur-[1px] flex items-center justify-center z-[300] pointer-events-none rounded-[14px]">
            <div className="bg-white dark:bg-neutral-900 border-2 border-black dark:border-neutral-700 p-4 rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
              <RefreshCw size={24} strokeWidth={2.5} className="animate-spin text-indigo-600 dark:text-indigo-400" />
            </div>
          </div>
        )}
